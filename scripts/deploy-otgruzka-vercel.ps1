...
# Ubrat privyazku k fst-uchet / fb-cell-admin (chuzhoj .vercel lomaet nikegeorgian)
$vercelDir = Join-Path $ProjectRoot '.vercel'
if (Test-Path $vercelDir) {
  Remove-Item $vercelDir -Recurse -Force -ErrorAction SilentlyContinue
  Write-Host '  Udalen staryj .vercel' -ForegroundColor DarkGray
}
