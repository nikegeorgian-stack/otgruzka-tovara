...
export function MaterialStockHint({
  order,
  warehouseItems,
  warehouseMovements,
  warehouseAccounting,
  ...
  }, [order, warehouseItems, warehouseMovements, warehouseAccounting])
  ...
  const unverified = rows.some((r) => r.stockTrust === 'unverified')
    ...
    if (unverified) {
      return (
        <p className="text-xs font-medium text-amber-800">
          {t('planner.material.stockUnverified')}
        </p>
      )
    }
    if (totalShort > 0) {
  ...
  if (unverified) {
    return (
      <div className="rounded-sm border border-amber-200 bg-amber-50/80 px-3 py-2 text-xs text-amber-950">
        <p className="font-semibold">{t('planner.material.stockUnverified')}</p>
        <p className="mt-1">{t('planner.material.stockUnverifiedHint')}</p>
      </div>
    )
  }
          ...
          <li key={row.itemId} className="flex flex-wrap justify-between gap-2">
            ...
            <span className="font-mono">
              {tf('planner.material.stockLine', {
                ...
                avail: formatQty(row.available ?? 0),
