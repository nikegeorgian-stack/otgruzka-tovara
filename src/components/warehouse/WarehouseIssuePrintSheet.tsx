...
import { formatReceiptMoney, type IssuePrintModel } from '@/lib/warehouse/printDocument'
...
export function WarehouseIssuePrintSheet({ model }: Props) {
  const { t } = useI18n()
  ...
  return (
    <article className="warehouse-receipt-page print-sheet-page">
      <div className="warehouse-receipt-content print-sheet-content">
        ...
        <h1 className="warehouse-receipt-title">{t('warehouse.print.issueTitle')}</h1>
        ...
        <dl className="warehouse-receipt-meta">
          ...
          {model.targetWarehouseName ? (
            <div>
              <dt>{t('warehouse.doc.targetWarehouse')}</dt>
              <dd>{model.targetWarehouseName}</dd>
            </div>
          ) : null}
          {model.brigade ? (
            <div>
              <dt>{t('warehouse.doc.brigade')}</dt>
              <dd>{model.brigade}</dd>
            </div>
          ) : null}
          {model.counterparty !== '—' ? (
        ...
        <div className="warehouse-receipt-signatures">
          <div className="warehouse-receipt-sig">
            <div className="warehouse-receipt-sig-role">{t('warehouse.doc.keeper')}</div>
            <div className="warehouse-receipt-sig-name">{model.issuedBy || '\u00A0'}</div>
          </div>
          <div className="warehouse-receipt-sig">
