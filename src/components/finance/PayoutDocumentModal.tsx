...
import { PrintPayoutDisbursementModal } from './PrintPayoutDisbursementModal'

type Props = {
  store: AppStore
  documentId?: string
  defaultMonth: string
  /** Предзаполнить назначение (напр. индивидуальная выплата). */
...
export function PayoutDocumentModal({
  store,
  documentId,
  defaultMonth,
  defaultPurpose,
  ...
  const existing = documentId ? payoutDocumentById(fin, documentId) : undefined
  ...
  function handleSaveDraft() {
    actions.onSavePayoutDocument(buildInput())
    onClose()
  }

  function handlePrepare() {
    const input = buildInput()
    actions.onSavePayoutDocument(input)
    actions.onPreparePayoutDocument(draftId)
    onClose()
  }

  function handleUnprepare() {
    if (!existing) return
    actions.onUnpreparePayoutDocument(existing.id)
    onClose()
  }

  function handleMarkPaid() {
    if (!existing) return
    actions.onPostPayoutDocument(existing.id)
    ...
    void import('@/lib/cloud/fstPushNotify').then(({ notifyEmployeesPush }) =>
      notifyEmployeesPush(
        ...
        ids,
        'Остаточная зарплата',
        'Вам провели выплату. Откройте «Моё», чтобы посмотреть сумму.',
        { kind: 'payout', month },
      ),
    )
    onClose()
  ...
  function handleVoid() {
    if (!existing || !window.confirm(t('fin.payDoc.confirmVoid'))) return
    actions.onVoidPayoutDocument(existing.id, voidReason.trim() || undefined)
    onClose()
  }

  function handleDeleteDraft() {
    if (!existing || !window.confirm(t('fin.payDoc.confirmDelete'))) return
    actions.onDeletePayoutDocumentDraft(existing.id)
    onClose()
  }
        ...
        <header className="flex shrink-0 flex-wrap items-start justify-between gap-3 border-b border-grid bg-stone-50 px-4 py-3">
          <div className="min-w-0">
            <h2 className="text-base font-bold text-ink sm:text-lg">
              {existing ? existing.number : t('fin.payDoc.new')}
            </h2>
        ...
        <div className="app-dialog-body space-y-4 px-4 py-4">
          ...
          <label className="block text-xs text-stone-500">
            ...
            <input
              ...
              value={purpose}
              disabled={readOnly || closed}
              placeholder={`${t('fin.payDoc.purposeDefault')} ${formatMonthTitle(month, locale)}`}
              className="mt-1 w-full rounded-sm border border-grid px-2 py-1.5 text-sm disabled:opacity-50"
            ...
            {docForPrint && docForPrint.lines.length > 0 && (
              <>
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    void runExport('payout_disbursement', store, {
                      month,
                      locale,
