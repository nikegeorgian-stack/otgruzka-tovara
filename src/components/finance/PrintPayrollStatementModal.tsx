...
import { PrintPayrollStatementSheet } from './PrintPayrollStatementSheet'
...
export function PrintPayrollStatementModal({
  store,
  month,
  rows,
  printLocale,
  ...
  async function handlePdf() {
    ...
    try {
      ...
      await exportPrintAreaToPdf(
        printRef.current,
        `payroll-statement-${month}.pdf`,
        { pageSelector: '.print-payroll-page', orientation: 'landscape' },
      )
    } finally {
      setPdfBusy(false)
    }
  ...
  return createPortal(
    ...
    >
      <div className="print-modal-toolbar no-print flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-stone-700 bg-stone-900 px-4 py-3 text-white">
        <div>
          <h2 className="text-lg font-bold">{t(printLocale, 'fin.printStatement')}</h2>
      ...
      <div className="print-modal-body">
        <div ref={printRef} id="print-area" className="print-area print-area--payroll">
          <PrintPayrollStatementSheet
            store={store}
            month={month}
            rows={rows}
