...
import { resolveAdvanceDocumentLines } from '@/lib/finance/advanceDocuments'
...
import { PrintAdvanceDisbursementSheet } from './PrintAdvanceDisbursementSheet'
...
export function PrintAdvanceDisbursementModal({
  store,
  document,
  printLocale,
  onClose,
  ...
  const allRows = useMemo(
    () =>
      resolveAdvanceDocumentLines(store, document).map((r) => ({
        lineId: r.line.id,
  ...
  return createPortal(
    ...
    >
      <div className="print-modal-toolbar no-print flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-stone-700 bg-stone-900 px-4 py-3 text-white">
        <div>
          <h2 className="text-lg font-bold">{t(printLocale, 'fin.advDoc.print')}</h2>
