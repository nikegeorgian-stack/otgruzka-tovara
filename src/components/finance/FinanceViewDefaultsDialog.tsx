...
import type { FinanceViewDefaults } from '@/lib/viewDefaults/types'

type TabDef = { id: FinanceSection; labelKey: string }

type Props = {
  tabs: TabDef[]
  initial: FinanceViewDefaults
  onSave: (defaults: FinanceViewDefaults) => void
  onClose: () => void
}

export function FinanceViewDefaultsDialog({ tabs, initial, onSave, onClose }: Props) {
  const { t } = useI18n()
  const [section, setSection] = useState<FinanceSection>(initial.section ?? 'dashboard')
  ...
  return (
    <AppDialog
      ...
      onClose={onClose}
      size="md"
      title={t('finance.defaults.title')}
      subtitle={t('finance.defaults.subtitle')}
      footer={
