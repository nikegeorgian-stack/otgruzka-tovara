...
import { AdvanceDocumentModal } from './AdvanceDocumentModal'
...
export function FinanceAdvanceDocumentsPanel({
  store,
  month,
  onMonthChange,
  documentActions,
  ...
  const docTotals = useMemo(() => {
    ...
    let ready = 0
    let draft = 0
    for (const d of listAdvanceDocuments(fin, { month, includeVoid: false })) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-bold text-ink">{t('fin.advDoc.panelTitle')}</h2>
