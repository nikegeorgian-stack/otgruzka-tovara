...
export function PrintPayoutDisbursementSheet({
  store,
  document: doc,
  printLocale,
  printRows,
  ...
  const resolved = resolvePayoutDocumentLines(store, doc)
  const rows =
    printRows ??
    resolved.map((r) => ({
      lineId: r.line.id,
  ...
  const title = `${t(printLocale, 'fin.payDoc.printTitle')} — ${doc.number}`
    ...
    <article className="print-sheet-page print-bank-transfer-page print-advance-doc-page">
      <div className="print-sheet-content">
        ...
        <p className="print-bank-purpose">
          {doc.purpose ||
            tf(printLocale, 'fin.payDoc.purposeDefaultLong', { month: monthTitle })}
          {' · '}
        ...
        <table className="print-table print-bank-transfer-table">
          <thead>
            <tr>
              ...
              <th className="print-th print-th-right">{t(printLocale, 'fin.payDoc.colAmount')}</th>
              {doc.method === 'bank' && (
                <th className="print-th print-th-left">
